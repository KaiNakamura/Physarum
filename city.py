...
def update_city_sizes():
    ...
    city_sizes = [(city.population / total_population) * 100 for city in selected_cities]  # Normalize out of 100
    return city_sizes
...
def plot_cities_on_map(cities, map_image_path):
    ...
    for city in selected_cities:
        ...
        text_plot = ax.text(lon, lat, f"{city.name}\nSize: {city.size:.2f}", color='black', fontsize=7)
        city_scatter_plots[city.name] = (scatter_plot, text_plot)
        city_labels.append(city.name)

    # Update plot sizes 
    def update_plot():
        city_sizes = update_city_sizes()  # Calculate new city sizes based on selected cities lists 
        print ("\nSelected cities: ")
    ...
    check = CheckButtons(rax, city_labels, [True] * len(city_labels))  # All cities selected on initialization
